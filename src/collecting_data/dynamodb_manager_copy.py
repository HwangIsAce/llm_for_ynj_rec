...
class DynamoDBManager:
        ...
        try:
            self.table = self.dynamodb.create_table(
                ...
                KeySchema=[
                    ...
                    {'AttributeName': 'category#place_name', 'KeyType': 'RANGE'}  # Sort Key
                ],
                AttributeDefinitions=[
                    {'AttributeName': 'region', 'AttributeType': 'S'},  # String 타입
                    {'AttributeName': 'category#place_name', 'AttributeType': 'S'}  # String 타입
                ],
    ...
    # example
    def query_turism_attraction_sites(self, main_region, prefix):
        if self.table is None:
            raise ValueError("테이블이 존재하지 않습니다. 테이블을 먼저 생성하세요.")
        
        response = self.table.query(
            KeyConditionExpression=Key("region").eq(main_region) & Key('category#place_name').begins_with(prefix)
        )
...
if __name__ == '__main__':
    ...
    # dynamodb_manager.delete_table("TourismAttraction") # delete a table